...
2. 依使用者下一步分類：
   ...
   - `🔎`：使用者需要理解重要背景或取捨。
   - `📦`：沒有未完成責任或重要學習，值得封存。
3. 若 agent 應繼續工作且使用者無須行動，不改名。Stop、idle、final answer 或只有本機 commit，都不足以標 `📦`。
4. 寫成以下精確格式：
   ...
   只能有一行及兩個半形 ` | ` 分隔符。目標 24–32 字，最多 40 字；進度語意必須符合狀態 emoji。

5. 若有 `codex_app__set_thread_title` 等 title tool，只呼叫一次。目前 task 省略 `threadId`；經授權替其他 task 改名時，使用步驟 1 取得的精確 ID 與 host。
6. 若沒有 title tool，回傳完整建議標題，請 CLI 使用者用 `/rename` 套用。不得重建舊 Stop hook、shell、app-server 或暫存檔 fallback。
