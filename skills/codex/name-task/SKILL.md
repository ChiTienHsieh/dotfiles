...
2. 若準備把目前 task 標題開頭的狀態 emoji 改為 `📦`：
   - 若 `wrap` 已完成封存前檢查並呼叫本 skill，直接沿用該次檢查結果，不得再次執行 `wrap`。
   - 其他情況先完整讀取 `~/dotfiles/skills/shared/wrap/SKILL.md`，請它只進行封存前檢查並回傳結果，不要更新標題；檢查通過才可繼續。
   - 替其他 task 改名時，不得沿用目前工作階段的檢查結果；只有重新讀取後確認該 task 已完成同等檢查，才可使用 `📦`。
3. 依使用者下一步分類：
