...
- 用原生 `list_threads` 取得分組、置頂、task 的 kind 與 host；需要專案資訊時用 `list_projects`。先查看使用者指定的工作；只有要求全面整理，或需要相關內容判斷狀態時才擴大查看。工具結果有限時說明覆蓋範圍。
...
- 搬動、排序、改名後重新讀取，確認實際結果。改名失敗依 `name-task` 讀取現況並回報，不重試；其他操作先核對現況，只重試尚未完成且仍在授權內的動作，持續失敗就回報精確目標與原因。

## 封存

整理分組不等於封存授權，鄰近項目也不自動納入封存範圍。只有使用者已明示要求封存，且目標 task 自身通過 `name-task` 所要求的同等封存前檢查，才用 `set_thread_archived`。
