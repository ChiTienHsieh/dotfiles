...
7. 套用標題：若執行環境有原生改標題工具，優先使用（見下方與 `runtimes/<環境>.md`）。否則執行 `scripts/rename-session.sh "<title>"` 取得建議標題。

   - Codex App：使用 `set_thread_title` tool（不需要 script）
   - 其他環境：script 只印出建議標題與手動 `/rename` 指令，**不會**對 pane `send-keys`。載入本 skill 或自動改名節點都不是 tmux mutation 授權；沒有目前這次 human 對 rename／tmux 的明確要求時，把建議標題交給使用者套用。不得把 task 內容或其他 prompt 文字嵌入自己的 pane。
