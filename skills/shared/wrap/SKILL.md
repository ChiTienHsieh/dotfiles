...
5. 若以上檢查確認沒有未完成責任或阻止封存的 blocker，且目前 surface 提供 `name-task` skill：
   - 若使用者直接呼叫 `wrap`，讀取並執行 `name-task`，把目前 task 標為 `📦`；`name-task` 不得再次執行 `wrap`。
   - 若本次 `wrap` 是由 `name-task` 呼叫，不得回呼 `name-task`；回傳完成證據，讓原本的 `name-task` 繼續。
