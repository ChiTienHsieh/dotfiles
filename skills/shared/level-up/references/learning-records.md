...
`.gitignore` 只允許公開索引、偏好與一層主題 Markdown；這是路徑篩選，不會判斷內容是否敏感。新增或修改公開紀錄時要檢查實際內容，不能直接把原始筆記搬回去。
...
3. 狀態只在主題檔記一次；INDEX 只放概念與連結。私人資料不得僅因較詳細就蓋過公開證據；有衝突時比較日期、來源與實際展示的能力，無法判斷就保留不確定。
...
- `mastered`：能自主應用或推導，必須寫清楚適用範圍；概念判斷不等於獨立實作。
- `familiar`：已有部分理解，但仍需引導、遷移驗證或實作；自述已會也先標明自述。
- `learning`：正在學，尚未完成相關理解。
- `gap`：已觀察到誤解、缺先備知識或反覆卡點。
- `skip_for_now`：使用者刻意暫緩某個學習主題。

證據保留原始日期，說明學習者實際做了什麼。拍板是設計決策，不自動證明理解；單題答對不自動升為 mastered。整理舊紀錄不是重新驗證，不能把日期更新成今天。
