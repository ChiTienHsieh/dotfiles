...
- 中文要像台灣人講話。禁止把類比收成沒人用的詞（例如「牆」）。場景可以打比方，結論用平常話講完。
- 2026-09-03：不用 ①②③ 這類圈圈數字（終端機裡糊成一團，尤其連在一起時），用「第 1 條」或 (1)。不用「判準」這個詞，講「判斷標準」。英文 jargon 出現前先查 allowlist；user 不認識的字（hunk、hedge）已進 reject。
- 2026-08-25：不要電報文。短，但要像午餐聊天把一句話講完；一個回合只講一件事。
