...
if command -v timeout >/dev/null 2>&1; then
  output=$(timeout "$TIMEOUT" "${HOOK_COMMAND[@]}" < "$TEST_INPUT" 2>&1)
  exit_code=$?
elif command -v gtimeout >/dev/null 2>&1; then
  output=$(gtimeout "$TIMEOUT" "${HOOK_COMMAND[@]}" < "$TEST_INPUT" 2>&1)
  exit_code=$?
else
  echo "⚠️  Warning: timeout/gtimeout not found; running without timeout enforcement."
  output=$("${HOOK_COMMAND[@]}" < "$TEST_INPUT" 2>&1)
  exit_code=$?
fi
set -e
