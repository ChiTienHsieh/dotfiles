...
if jq -e '.hooks? | type == "object"' "$HOOKS_FILE" >/dev/null; then
  HOOKS_JQ='.hooks'
  echo "Detected plugin hooks wrapper: using .hooks"
fi
...
for event in $(jq -r "$HOOKS_JQ | keys[]" "$HOOKS_FILE"); do
  found=false
...
for event in $(jq -r "$HOOKS_JQ | keys[]" "$HOOKS_FILE"); do
  hook_count=$(jq -r "$HOOKS_JQ | .\"$event\" | length" "$HOOKS_FILE")

  for ((i=0; i<hook_count; i++)); do
    # Check matcher exists
    matcher=$(jq -r "$HOOKS_JQ | .\"$event\"[$i].matcher // empty" "$HOOKS_FILE")
    if [ -z "$matcher" ]; then
    ...
    hooks=$(jq -r "$HOOKS_JQ | .\"$event\"[$i].hooks // empty" "$HOOKS_FILE")
    ...
    hook_array_count=$(jq -r "$HOOKS_JQ | .\"$event\"[$i].hooks | length" "$HOOKS_FILE")

    for ((j=0; j<hook_array_count; j++)); do
      hook_type=$(jq -r "$HOOKS_JQ | .\"$event\"[$i].hooks[$j].type // empty" "$HOOKS_FILE")
      ...
      if [ "$hook_type" = "command" ]; then
        command=$(jq -r "$HOOKS_JQ | .\"$event\"[$i].hooks[$j].command // empty" "$HOOKS_FILE")
        if [ -z "$command" ]; then
        ...
        prompt=$(jq -r "$HOOKS_JQ | .\"$event\"[$i].hooks[$j].prompt // empty" "$HOOKS_FILE")
        if [ -z "$prompt" ]; then
      ...
      # Check timeout
      timeout=$(jq -r "$HOOKS_JQ | .\"$event\"[$i].hooks[$j].timeout // empty" "$HOOKS_FILE")
