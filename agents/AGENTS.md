...
## 執行任務
- 清楚、安全的修改任務一路完成實作、適當驗證、`commit`、review、`push`、PR 與 CI。一般改動自行 review diff，必要的獨立 review 依下節與 repo 規定。一般實作選擇自行判斷，已授權的步驟不重複確認；需要使用者決定範圍、安全或機密疑慮、破壞性 Git 操作、`force-push`、付費或資料遺失風險時才停下相關動作，先完成不受影響的工作。
