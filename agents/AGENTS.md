...
- 寫給人或另一個 agent 的文字先講結果，再補讀者需要的脈絡。用自然、完整的句子，不加公文腔、客套或裝飾句；比較、步驟或多項資訊需要時才用列表與表格。
...
## 執行任務
- 清楚、安全的修改任務一路完成實作、適當驗證、`commit`、review、`push`、PR 與 CI。一般實作選擇自行判斷，已授權的步驟不重複確認；需要使用者決定範圍、破壞性 Git 操作、機密、`force-push`、付費或資料遺失風險時才停下相關動作，先完成不受影響的工作。
- 驗證範圍依改動與風險決定，完成專案必要 checks；通過後，只有新變更、失敗或未解疑慮才擴大或重跑。新增測試應驗證有意義的行為，不為可逆的小改動新增只比對實作文字的測試。
- 安全指令被 sandbox、權限、Keychain 或網路擋住時，用合適的 escalation 重試；高風險指令不自行 escalation。保留使用者或其他 agent 的變更；收尾仍 dirty 時說明歸屬與可行整理方式，不自行 discard。
