...
## 目前訂閱狀態（user 於 2026-08-02 確認）

- **Codex＝主力**：200 USD/month。重活、一般 review、read-only fan-out 預設優先使用。
- **Claude Code＝輔助**：20 USD/month。保留給 Claude-specific workflow、controller judgment，或需要不同 provider 的獨立 review。
- **Grok＝輔助**：30 USD/month。保留給不同 provider 的 second opinion，或 Codex/Claude 都卡住時的 fallback。

月費只決定預設路由，不代表當下 quota。長任務與 limit error 仍用 `quota`
...
- **bounded read-only 工作** → controller 是 Codex 時優先用 native subagent；controller 是 Claude Code、或需要乾淨 CLI process / structured output 時，走 shared `headless-agents` 的 hardened Codex wrapper。
- **Claude Code**保留 product judgment、Claude-specific 行為驗證，以及 cross-provider review；不要用 20 USD quota 承擔可由 Codex 完成的機械型長迴圈。
