...
- headless CLI worker 要改檔，必須套 `headless-cli-agents` skill 的 sandbox profile（寫入限 worktree、deny 讀 credential、能關網路就關）；沒有 profile 就只准 read-only。human 已明確授權 tmux 時，才依 `tmux-orchestration` 的權限、觀察與 cleanup 合約執行。

## 路由規則（用角色寫，換 provider 時規則不動）

- **重活實作**（bulk edits、多檔改動、長時間跑）→ 目前 runtime 內建、能安全改檔的 subagent，或套 sandbox profile 的 headless CLI worker；選餘量最多的那家，headless 是選項不是義務（Codex 週配額常先用完，回到 Claude 內建 subagent 很正常）。主 agent 保留 scope、判斷、驗收與 git ownership：自己重跑驗證指令，worker 回報「完成」但 diff 是空的一律當拒絕處理。
