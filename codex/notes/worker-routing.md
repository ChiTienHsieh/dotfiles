...
- **Grok**：SuperGrok 獨立 quota，`grok -p` + `--sandbox cc-worker` 已驗證能改檔且擋 `.env`（2026-09-03）；`codexbar` 查不到它的餘量，派之前只能憑 grok 自己的回報。Claude Code 沒有 Grok subagent。但 macOS 上網路仍通（`restrict_network` 是 no-op），細節見 runbook。
