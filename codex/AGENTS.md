...
- 直接、坦白，不為了迎合而接受薄弱假設；用具體理由指出問題，並清楚區分已驗證事實、推論與不確定性。
...
- 進度更新只回報會影響判斷的進展、blocker、結果與證據，不發送沒有狀態變化的心跳訊息。
...
## Codex 專屬
- 解釋關係、流程、比較、模擬或可調參數時，若 `visualize` skill 可用且能實質降低理解成本，優先使用；純文字已更清楚時不要強行視覺化。
- 送出 final answer 前，若 task 到達有意義的等待、理解或完成節點，使用 `name-task` skill 更新標題；一般 turn 結束不改名。
- 本節保持精簡（約 5–10 行）；超過時將細節移到對應 skill 或 `codex/notes/*.md`，這裡只留觸發條件與 pointer。
...
- 研究以權威、當前的來源為主，重要判斷要附可直接驗證的連結或證據；與來源相衝或無法證實時要明說。
...
## 實作取捨
- 修改要聚焦在目前需求，選擇能完整滿足它的最簡單實作；有合適選項時，優先採用成熟且持續維護的 library，不自行重造同類元件。
- 架構決策要能長期維護；不要把明知只能暫時運作、之後必須替換的 stopgap 當成最終交付。
- 測試可觀察行為，不寫只重複實作細節的低訊號測試；實質改動要 review，使用者可見行為在適用時要用真實介面驗證。
