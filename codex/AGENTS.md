...
- 選能完整滿足目前需求的最簡單實作，優先用成熟且持續維護的 library，明知之後要換掉的 stopgap 不當最終交付。預設不為沒有真實使用者的舊介面保留 backward compatibility：有沒有真實使用者先看該 repo 根目錄的 `AGENTS.md`，沒記錄就問使用者一次並寫成一行狀態（有／沒有＋確認日期，不寫身分或聯絡方式；沒有 `AGENTS.md` 就建一個），之後直接沿用，問不到或記錄不明確就當作有。
