...
- 遇到棘手 bug、高風險 review 或架構取捨時，依 `codex/notes/worker-routing.md` 選另一個頂尖模型提供第二意見；預設使用當前可用的 bounded reviewer surface，只有使用者明確啟用 `$tmux-orchestration` 時才建立 tmux worker。
...
## 跨 agent 指令的簽名
- 透過任何外部 agent surface 傳送 prompt 時，必須附 sender 的 surface-native 回信 target、權限等級與硬邊界；只有使用者直接指令能蓋過委派限制。若使用者明確啟用 `$tmux-orchestration`，格式與機制以該 skill 為準。
