...
## 回覆
- 一律用自然的台灣繁體中文回覆，不常見的詞順手簡短解釋；除非任務明確要求本地化，不翻譯程式碼裡的識別字、檔案路徑、指令、設定鍵、model ID 或 UI 原文標籤。英文詞彙與台灣用語替換表都在 `~/dotfiles/hooks/jargon-allowlist.yml`（pre-commit 會擋），不確定的詞 `grep -i "word" hooks/jargon-allowlist.yml` 看它屬於哪個 section。
...
## 環境
- 技術背景：Python / FastAPI / LLM；macOS M1/M2；Python 用 uv；bun 優先於 npm。處理 clawd-vm、Clawd/OpenClaw、Iris/Hermes、SSH 或 GitHub AI 帳號前，先讀本機 SSOT `~/.local/share/machine/machine.md`（`~/.codex/machine.md` 是它的 symlink，write-guard 會擋 symlink 所以直接改本體；舊路徑 `~/.config/machine.md` 只是導向 stub；裡面不放 token 或 private key）。調查 Codex CLI 設定或 TUI 功能前，先讀 `codex/notes/codex-cli.md` 裡已知的限制與死路。

## 執行任務
- 清楚、安全的任務一路做完修正、測試、`commit`、`push`，開 PR 後自己追 CI；安全的指令被 sandbox、權限、Keychain 或網路擋住就先用合適的 escalation 重試，只有遇到破壞性 Git 操作、機密、`force-push`、付費或資料遺失風險才停下。收尾時 worktree 仍 dirty 就列整理選項（commit/push、拆分 stage、stash、經同意 discard、維持 dirty），不自行清掉使用者沒交代的變更。
...
- 選能完整滿足目前需求的最簡單實作，優先用成熟且持續維護的 library，明知之後要換掉的 stopgap 不當最終交付。預設不為沒有真實使用者的舊介面保留 backward compatibility：有沒有真實使用者先看該 repo 根目錄的 `AGENTS.md`，沒記錄就問使用者一次並寫成一行狀態（有／沒有＋確認日期，不寫身分或聯絡方式），問不到或記錄不明確就當作有。
