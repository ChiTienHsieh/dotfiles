...
- 若 dirty changes 不屬於目前任務且來源不明，先凍結所有 Git 寫入，依 `skills/shared/tidy-workspace/references/dirty-worktree-ownership.md` 派一個唯讀 subagent 用 Codex App thread tools 查找負責的 task 並協商隔離；無法用精確證據確認負責者時就停止，不猜測、不廣播，也不碰那些變更。
