...
from request_thread_title import clear_title_request, take_title_request
...
Use the request helper below exactly once, replacing only <TITLE>. Quote the title as one shell argument:
{request_command}

The request helper stages one validated title in a private temporary file. When this Stop-hook continuation finishes, the hook applies it through only the stable Codex app-server method `thread/name/set`; this is the non-interactive equivalent of the CLI user's `/rename` command.

This automation may set a title only. Never archive, unarchive, delete, or otherwise change thread lifecycle state. 📦 is merely a recommendation that the user may archive later. If the request helper is unavailable, skip quietly. If it exists but the one request fails, mention one concise non-blocking warning. This continuation was already created by the Stop hook; do not request another continuation or retry.
"""
...
def request_command_for(session_id: object) -> str:
    thread_id = str(session_id or "")
    helper = '$HOME/.codex/hooks/request_thread_title.py'
    return (
        f'/usr/bin/env python3 "{helper}" '
        f"--thread-id {shlex.quote(thread_id)} --title '<TITLE>'"
    )
...
def build_stop_result(payload: dict[str, object]) -> dict[str, object]:
    ...
    thread_id = str(payload.get("session_id") or "")
    try:
        clear_title_request(thread_id)
    except (OSError, ValueError, RuntimeError):
        pass
    reasons = [
        TITLE_CHECKPOINT.format(request_command=request_command_for(thread_id))
    ]
