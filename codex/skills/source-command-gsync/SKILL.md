...
- `the user-provided request text` — optional branch name. If empty, use current branch.
...
If `the user-provided request text` is provided and differs from current branch, check that branch instead.

### Phase 2: Analyze Diff

If there are incoming remote commits OR local uncommitted changes, spawn a small review subagent to:
...
### Local Changes
<diff summary from review subagent, or "Clean working tree">
...
Before recommending any push action, spawn a small review subagent to quickly scan the staged/committed changes for:
...
Ask the user to choose an action. If a structured user-input tool is available, use it; otherwise ask a concise plain-text question. Rules:
